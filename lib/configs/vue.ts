...
export function vue(options: ConfigOptions): Linter.Config[] {
	...
	const linterRules: Linter.Config[] = [{
	...
	const formatterRules: Linter.Config[] = [{
		files: GLOB_FILES_VUE,
	...
	return [
		...(options.vueIsTypescript ? [typescriptConfig] : []),
		...(options.linting ? linterRules : []),
		...(options.formatting ? formatterRules : []),
	]
}
